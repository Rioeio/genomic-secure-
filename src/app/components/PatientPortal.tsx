...
export function PatientPortal() {
  ...
  const toggleConsent = (projectId: string) => {
    ...
    const studyTitle = mockStudies.find(s => s.id === projectId)?.title || 'Study';
    ...
    const actionText = isRevoking ? 'Revoked Access for' : 'Granted Consent for';
    setNotification(`Consent Manager Agent: ${actionText} "${studyTitle}". Zero-knowledge matrix updated instantly across all hospital nodes.`);
  ...
  return (
    <div className="flex-1 overflow-auto bg-zinc-50/50 relative">
      ...
      {notification && (
        <div className="fixed bottom-6 right-6 z-50 max-w-md bg-zinc-900 text-white p-4 rounded-lg shadow-2xl border border-zinc-800 flex items-start gap-3 animate-in fade-in slide-in-from-bottom-4 duration-300">
          <UserCheck className="w-5 h-5 text-teal-400 shrink-0 mt-0.5" />
          <div className="flex-1">
            <p className="text-xs font-semibold text-teal-300 uppercase tracking-wider mb-1">Agent Event Logged</p>
            <p className="text-xs text-zinc-200 leading-relaxed">{notification}</p>
          </div>
        </div>
      )}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <div className="px-3 py-1.5 bg-zinc-100 rounded-md border border-zinc-200">
                <p className="text-xs text-zinc-700 font-medium tracking-wide">ID: <span className="font-mono font-bold text-zinc-900">{mockPatient.anonymizedId}</span></p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
          ...
          <div className="bg-white rounded-lg p-5 border border-zinc-200 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-semibold tracking-tight text-emerald-600 mt-2">{consentedProjects.length}</p>
              </div>
        ...
        <div className="bg-white rounded-lg border border-zinc-200 p-6 mb-8 shadow-sm">
          <div className="flex items-center gap-2.5 mb-5 border-b border-zinc-100 pb-4">
            ...
            <h2 className="text-lg font-semibold tracking-tight text-zinc-900">My Genomic Biobank Pass</h2>
          </div>
          ...
          <div className="mt-5 p-4 bg-emerald-50/50 rounded-md border border-emerald-200">
            <p className="text-xs text-emerald-800 leading-relaxed">
              <span className="font-semibold text-emerald-900">Zero Raw Exposure Guarantee:</span> Your raw genomic sequencing files remain encrypted within your local hospital enclave. Only noise-injected gradients evaluated by the Privacy Guard Agent leave the hospital.
            </p>
          </div>
        </div>
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <button
                      ...
                      className="shrink-0 px-4 py-2 bg-white text-rose-700 border border-rose-200 rounded-md hover:bg-rose-50 transition-colors text-sm font-semibold shadow-xs cursor-pointer"
                    >
                      Revoke Access
                    </button>
                  </div>
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1 pr-6">
                      ...
                      <div className="flex items-center gap-6 text-sm">
                        ...
                        <span className="text-zinc-500">
                          <span className="font-medium text-zinc-700">Privacy Protection:</span> {project.privacyLevel}
                        </span>
                      </div>
                    </div>
                    <button
                      onClick={() => toggleConsent(project.id)}
                      className="shrink-0 px-4 py-2 bg-zinc-900 text-white rounded-md hover:bg-zinc-800 transition-colors text-sm font-semibold shadow-xs cursor-pointer"
                    >
                      Give Consent
                    </button>
                  </div>
            ...
            {mockPatient.dataAccessLog.map((event) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className="text-xs text-zinc-500 font-mono">{event.accessDate}</span>
                </div>
